...
<- ЭТО ПРОСТО. Настраиваете один раз - используете всегда. Для создания загрузочного образа с любым дистрибутивом нужно выполнить всего одну команду.

<- ЭТО БЫСТРО. Дистрибутив загружается в оперативную память. Оперативная память - это скорость и экономия ресурса накопителей.

<- ЭТО БЕЗОПАСНО. Выполните "rm -rf /". Нажмите Reset. Операционная система вернётся в исходное состояние.

<- ЭТО НАДЁЖНО. Резервное копирование больше не нужно. 

-> Какие минусы использовать booty?

<- Отключение питания компьютера приведёт к потере всех данных в оперативной памяти. Храните данные в "облаке".
