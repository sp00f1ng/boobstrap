...
5) ЭТО БЕЗПРОБЛЕМНО. Переустановка, обновление или восстановление системы решается кнопкой Reset.
...
В каких ситуациях следует использовать booty?

Ситуация А) Славик купил VDS, установил Debian / Ubuntu / CentOS и настроил систему. Всё было хорошо. Но. Плохое качество услуг. Взломали сервер. Захотел другой сервер. Захотел другого хостера. Нужно сохранить все данные, перенести, заново развернуть, заново настроить. Бросить старую VDS и найти новую VDS - это как развод: долго, проблематично, затратно. Славик несчастлив. Не будь как Славик.

Ситуация Б) Ванечка купил VDS, но к серверу он даже не прикоснулся. Используя booty, всего одной командой в консоли, Ванечка создал загрузочный образ со своим любимым дистрибутивом, загрузил образ на VDS и загрузил систему прямо из образа. Система настроена на локальном компьере и сразу после загрузки решала задачи. Ванечке не нужно делать бэкапы, ведь образ с системой и есть бэкап. Когда нужно обновить систему, Ванечка зальёт обновлённый образ на VDS и одним нажатием кнопки ПЕРЕЗАГРУЗИТЬ обновит систему на VDS. Ванечка просто так по приколу сделал mkswap /dev/sda && swapon /dev/sda. Ванечка меняет VDS как женщин: не понравилась одна VDS, пошёл к другой VDS. Ванечка счастлив. Будь как Ванечка.
...
wget (для загрузки исходного кода ядра)
...
При наличии всех установленных зависимостей простой запуск команды booty build создаст полностью работоспособный загрузочный образ операционной системы.

Проверить работоспособность образа можно в виртуальной машине qemu-system-x86_64 -enable-kvm -cdrom BOOT-$(uname -m).ISO

Примеры использования:

# booty build boot.iso gentoo-stage3-distfiles/
...
# booty build boot.iso gentoo-stage3-distfiles/ documents-and-settings/
...
# booty build boot.iso gentoo-stage3-distfiles/ --profile initramfs
...
# booty build ОБРАЗ ДИРЕКТОРИЯ... ПАРАМЕТРЫ
