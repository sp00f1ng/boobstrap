...
Баг архитектурный. Когда система долго работает, размер /var/log может разрастись до больших размеров из-за китайских бонетов, брутфорсящих SSH. Из-за этого на первый взгляд стабильная, работающая система просто зависает, потому что оперативная память закончилась.

Решение проблемы очень простое, монтировать /var/log отдельным tmpfs разделом с жёстким лимитом в условные 10мб. Для хранения лог-файлов использовать внешнее хранилище.
...
Вы можете поддержать проект (и меня в том числе) материально.

Сбербанк: **4274 3200 2680 5393**

PayPal: paypal.me/spfng

WebMoney: Z274394626642 / **R660212101597** / P648692378521

Не забудьте оставить подпись и свой никнейм.
